...
{
    public partial class Splash : WorktipsWalletForm
    {
        ...
        private void SplashBackgroundWorker_DoWork(object sender, DoWorkEventArgs e)
        {
                    ...
                    if (System.IO.File.Exists("walletd.log")) 
                    {
                        using (StreamReader sr = new StreamReader("walletd.log"))
                        {
                                ...
                                contents.Contains("Corruption: no meta-nextfile entry in descriptor"))
                            {
                                ...
                                {
                                    System.Diagnostics.Process.Start("https://github.com/vordas/worktips");
                                }
                        ...
                        using (var sr = new StreamReader(fs))
                        {
                            while (!sr.EndOfStream)
                            {
                                ...
                                if (lastLine.Contains("The password is wrong") || lastLine.Contains("Restored view public key doesn't correspond to secret key"))
                                {
                                    ...
                                    MessageBox.Show("The password is incorrect!", "WorktipsCoin Wallet");
            ...
            catch (Exception ex)
            {
                ...
                MessageBox.Show("Daemon Connection Failed: " + ex.Message, "WorktipsCoin Wallet", MessageBoxButtons.OK, MessageBoxIcon.Error);
                Environment.Exit(3);
            }
        }
